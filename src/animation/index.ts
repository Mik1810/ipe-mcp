...
const usedIds = (document: DocumentIR) =>
  new Set(
...
function assertBox(box: AnimationBox, label: string): void {
  if (
...
function commit(document: DocumentIR, candidate: DocumentIR): void {
...
function setHandout(views: View[], policy: HandoutPolicy): void {
...
function easing(kind: SemanticEasing, t: number): number {
...
function translated(matrix: Matrix | undefined, pose: Pose): Matrix {
  return multiplyMatrices(
    translationMatrix(pose.x, pose.y),
    matrix ?? IDENTITY_MATRIX,
  );
}
function renumber(page: Page): void {
...
function preserveLegacyVisibility(
  views: readonly View[],
