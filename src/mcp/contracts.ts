...
export const operationSchema = z.discriminatedUnion("op", [
  ...
  z.object({ op: z.literal("group_objects").describe("Replace contiguous same-layer objects with a group."), pageId: entityIdSchema.describe("Owning page."), objectIds: z.array(entityIdSchema).min(2).max(maxIds).describe("Distinct contiguous object IDs in global z-order."), clip: pathSchema.optional().describe("Optional closed clip path."), url: z.string().max(maxPath).refine((value) => {
    try { const protocol = new URL(value).protocol; return protocol === "http:" || protocol === "https:"; }
    catch { return false; }
  }, "group hyperlink URLs must be absolute http(s)").optional().describe("Optional group hyperlink URL."), decoration: z.string().max(maxStyle).optional().describe("Optional defined decoration style.") }).strict(),
