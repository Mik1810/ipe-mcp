...
export const hintSchema = z.object({
  ...
  code: z.string().min(1).max(80).describe("Stable machine-readable hint code."),
  message: z.string().min(1).max(500).describe("Sanitized corrective guidance."),
}).strict().describe("One bounded model-facing recovery or workflow hint.");

export const publicErrorSchema = z.object({
  code: z.string().min(1).max(80).describe("Stable discriminant such as REVISION_CONFLICT or INVALID_ARGUMENT."),
  message: z.string().min(1).max(500).describe("Safe explanation without sensitive paths or document content."),
  retryable: z.boolean().describe("Whether retrying after the stated correction can succeed."),
  correction: z.string().min(1).max(500).describe("Concrete next action for recovery."),
...
export const resultSchema = z.object({
  ...
  kind: z.string().min(1).max(80).describe("Stable tool-result kind."),
  summary: z.string().min(1).max(1000).describe("Short sanitized outcome suitable for model context."),
...
const pagePatch = z.object({
  name: z.string().min(1).max(120).optional().describe("Unique PDF destination name; omitted leaves it unchanged."),
  title: z.string().max(500).optional().describe("Visible page title metadata; omitted leaves it unchanged."),
  section: z.string().max(500).optional().describe("Page section metadata; omitted leaves it unchanged."),
  subsection: z.string().max(500).optional().describe("Page subsection metadata; omitted leaves it unchanged."),
  notes: z.string().max(4000).optional().describe("Presenter notes shared by the page views; omitted leaves them unchanged."),
...
const pathStyle = z.object({
  stroke: z.string().max(80).optional().describe("Numeric color or defined Ipe color style name."),
  fill: z.string().max(80).optional().describe("Numeric color or defined Ipe color style name; omit for no fill."),
  pen: z.union([z.string().max(80), positive("Numeric pen width in bp.")]).optional().describe("Positive pen width or defined Ipe pen style."),
  dash: z.string().max(160).optional().describe("Defined dash style or valid Ipe/PDF dash pattern."),
  opacity: z.string().max(80).optional().describe("Defined Ipe opacity style."),
...
export const pathSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.enum(["polyline", "polygon"]).describe("Open polyline or closed polygon."), points: z.array(point).min(2).max(1000).describe("Ordered path vertices; polygon closes automatically."), ...styled }).strict(),
  ...
  z.object({ kind: z.enum(["uniform", "catmull-rom", "clothoid"]).describe("Supported multi-point spline family."), points: z.array(point).min(2).max(1000).describe("Ordered spline control points."), closed: z.boolean().optional().describe("Close the spline when supported."), tension: finite("Spline tension; used only by catmull-rom.").optional(), ...styled }).strict(),
...
const objectReplacementSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("text").describe("Replace with bounded LaTeX-backed text."), text: z.string().min(1).max(8000).describe("Complete replacement text content."), position: point.describe("Replacement text reference position."), width: positive("Minipage width in bp.").optional(), stroke: z.string().max(80).optional().describe("Numeric color or defined text color style."), size: z.union([positive("Numeric font size."), z.string().max(80)]).optional().describe("Numeric size or defined text-size style.") }).strict(),
  z.object({ kind: z.literal("symbol_use").describe("Replace with a native symbol use."), name: z.string().min(1).max(120).describe("Exact symbol name, including parameter suffix."), position: point.optional().describe("Optional symbol reference position."), stroke: z.string().max(80).optional().describe("Stroke parameter accepted only by symbols declaring s."), fill: z.string().max(80).optional().describe("Fill parameter accepted only by symbols declaring f."), size: z.union([positive("Numeric symbol size."), z.string().max(80)]).optional().describe("Size parameter accepted only by symbols declaring x.") }).strict(),
...
const styleDefinitionSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("color").describe("Named grayscale or RGB color."), name: z.string().regex(/^[A-Za-z][^\s]*$/u).max(80).describe("Style name."), value: z.union([z.number().min(0).max(1), z.tuple([z.number().min(0).max(1), z.number().min(0).max(1), z.number().min(0).max(1)])]).describe("Gray component or RGB components in [0,1].") }).strict(),
  z.object({ kind: z.enum(["pen", "symbolsize", "arrowsize", "textsize"]).describe("Named numeric or symbolic size style."), name: z.string().regex(/^[A-Za-z][^\s]*$/u).max(80).describe("Style name."), value: z.union([positive("Positive numeric size."), z.string().min(1).max(80)]).describe("Numeric size or accepted Ipe size expression.") }).strict(),
  z.object({ kind: z.literal("dashstyle").describe("Named Ipe/PDF dash pattern."), name: z.string().regex(/^[A-Za-z][^\s]*$/u).max(80).describe("Style name."), value: z.string().min(1).max(160).describe("Ipe/PDF dash syntax such as [4 2] 0.") }).strict(),
  z.object({ kind: z.literal("opacity").describe("Named opacity."), name: z.string().regex(/^[A-Za-z][^\s]*$/u).max(80).describe("Style name."), value: z.number().min(0.001).max(1).describe("Opacity in [0.001,1].") }).strict(),
  z.object({ kind: z.literal("symbol").describe("Reusable path-backed Ipe symbol."), name: z.string().min(1).max(120).describe("Native symbol name, including any parameter suffix."), path: pathSchema.describe("Path payload compiled as the symbol body.") }).strict(),
...
const layoutSpec = z.discriminatedUnion("primitive", [
  z.object({ primitive: z.enum(["row", "column"]).describe("Linear row or column layout."), container: box.describe("Container for all items."), items: z.array(layoutItem).min(1).max(128).describe("Objects and current bounds in desired order."), gap: z.number().min(0).finite().optional().describe("Gap between items in bp."), padding: z.number().min(0).finite().optional().describe("Uniform container padding in bp."), mainAlign: z.enum(["start", "center", "end", "space-between"]).optional().describe("Alignment along the layout direction."), crossAlign: z.enum(["start", "center", "end", "stretch"]).optional().describe("Alignment across the layout direction.") }).strict(),
  z.object({ primitive: z.literal("grid").describe("Deterministic grid layout."), container: box.describe("Grid container."), items: z.array(layoutItem).min(1).max(128).describe("Objects and current bounds in cell order."), columns: z.number().int().min(1).max(64).describe("Fixed grid column count."), rowGap: z.number().min(0).finite().optional().describe("Gap between rows."), columnGap: z.number().min(0).finite().optional().describe("Gap between columns.") }).strict(),
  z.object({ primitive: z.literal("stack").describe("Overlapping stack layout."), container: box.describe("Stack container."), items: z.array(layoutItem).min(1).max(128).describe("Objects and current bounds."), horizontalAlign: z.enum(["left", "center", "right"]).optional().describe("Horizontal placement inside the stack."), verticalAlign: z.enum(["bottom", "center", "top"]).optional().describe("Vertical placement inside the stack.") }).strict(),
]).describe("M3 semantic layout request converted into atomic object transforms.");

export const operationSchema = z.discriminatedUnion("op", [
  z.object({ op: z.literal("set_metadata").describe("Update document metadata."), title: z.string().max(500).optional().describe("Document title."), author: z.string().max(500).optional().describe("Document author.") }).strict(),
  z.object({ op: z.literal("add_page").describe("Add a page with its initial layer and view."), name: z.string().min(1).max(120).optional().describe("Unique page destination name."), title: z.string().max(500).optional().describe("Page title.") }).strict(),
  ...
  z.object({ op: z.literal("reorder_pages").describe("Replace global page order."), pageIds: z.array(entityIdSchema).min(1).max(512).describe("Every page ID exactly once, in desired order.") }).strict(),
  z.object({ op: z.literal("add_layer").describe("Add a layer to a page."), pageId: entityIdSchema.describe("Owning page."), name: z.string().regex(/^\S+$/u).max(120).describe("Unique whitespace-free layer name."), intentional: z.boolean().optional().describe("Required true for reserved Ipe layers such as BBOX.") }).strict(),
  z.object({ op: z.literal("update_layer").describe("Update layer editing semantics."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Layer to update."), name: z.string().regex(/^\S+$/u).max(120).optional().describe("New unique layer name."), edit: z.boolean().optional().describe("Whether native Ipe permits editing this layer."), snap: z.enum(["never", "visible", "always"]).optional().describe("Native snapping policy for the layer.") }).strict(),
  ...
  z.object({ op: z.literal("reorder_layers").describe("Replace layer list order without changing object z-order."), pageId: entityIdSchema.describe("Owning page."), layerIds: z.array(entityIdSchema).min(1).max(128).describe("Every layer ID exactly once, in desired order.") }).strict(),
  z.object({ op: z.literal("add_view").describe("Add a static page view."), pageId: entityIdSchema.describe("Owning page."), name: z.string().min(1).max(120).optional().describe("Optional unique destination name."), visibleLayerIds: z.array(entityIdSchema).min(1).max(128).describe("Layers visible in this static view."), activeLayerId: entityIdSchema.describe("Visible active layer used for editing."), marked: z.boolean().optional().describe("Whether this view is selected for handouts.") }).strict(),
  z.object({ op: z.literal("update_view").describe("Update static view visibility/editing semantics."), pageId: entityIdSchema.describe("Owning page."), viewId: entityIdSchema.describe("View to update."), visibleLayerIds: z.array(entityIdSchema).min(1).max(128).optional().describe("Complete replacement visible-layer set."), activeLayerId: entityIdSchema.optional().describe("Active layer, which must remain visible."), marked: z.boolean().optional().describe("Whether this view is selected for handouts.") }).strict(),
  ...
  z.object({ op: z.literal("reorder_views").describe("Replace view order and therefore PDF expansion order."), pageId: entityIdSchema.describe("Owning page."), viewIds: z.array(entityIdSchema).min(1).max(256).describe("Every view ID exactly once, in desired order.") }).strict(),
  z.object({ op: z.literal("add_rectangle").describe("Convenience rectangle insertion."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Target layer; z-order remains global."), x: finite("Left x coordinate."), y: finite("Bottom y coordinate."), width: positive("Rectangle width."), height: positive("Rectangle height."), stroke: z.string().max(80).optional().describe("Numeric color or defined style name."), fill: z.string().max(80).optional().describe("Numeric color or defined style name."), position: position.optional() }).strict(),
  z.object({ op: z.literal("add_segment").describe("Convenience segment insertion."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Target layer."), from: point.describe("Segment start."), to: point.describe("Segment end."), stroke: z.string().max(80).optional().describe("Numeric color or defined style name."), position: position.optional() }).strict(),
  ...
  z.object({ op: z.literal("add_text").describe("Insert bounded LaTeX-backed text."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Target layer."), text: z.string().min(1).max(8000).describe("Untrusted LaTeX fragment subject to native policy."), position: point.describe("Text reference position."), width: positive("Minipage width in bp.").optional(), stroke: z.string().max(80).optional().describe("Numeric color or defined text color style."), size: z.union([positive("Numeric font size."), z.string().max(80)]).optional().describe("Numeric size or defined text-size style."), positionInZOrder: position.optional() }).strict(),
  z.object({ op: z.literal("add_image").describe("Validate/deduplicate a bounded bitmap asset and insert its image object."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Target layer."), mediaType: z.enum(["image/png", "image/jpeg"]).describe("Encoded bitmap media type."), dataBase64: z.string().min(4).max(12_000_000).describe("Base64 bitmap payload; decoded bytes are preflighted before allocation."), target: box.describe("Image target box."), fit: z.enum(["contain", "cover", "stretch"]).optional().describe("Fit policy; cover emits a clipped group."), opacity: z.string().max(80).optional().describe("Defined Ipe opacity style."), position: position.optional() }).strict(),
  z.object({ op: z.literal("add_symbol_use").describe("Insert a native use object for a defined or built-in symbol."), pageId: entityIdSchema.describe("Owning page."), layerId: entityIdSchema.describe("Target layer."), name: z.string().min(1).max(120).describe("Exact symbol name, including parameter suffix."), position: point.optional().describe("Optional symbol reference position."), stroke: z.string().max(80).optional().describe("Stroke parameter accepted only by symbols declaring s."), fill: z.string().max(80).optional().describe("Fill parameter accepted only by symbols declaring f."), size: z.union([positive("Numeric symbol size."), z.string().max(80)]).optional().describe("Size parameter accepted only by symbols declaring x."), positionInZOrder: position.optional() }).strict(),
  ...
  z.object({ op: z.literal("group_objects").describe("Replace contiguous same-layer objects with a group."), pageId: entityIdSchema.describe("Owning page."), objectIds: z.array(entityIdSchema).min(2).max(128).describe("Distinct contiguous object IDs in global z-order."), clip: pathSchema.optional().describe("Optional closed clip path."), url: z.string().url().max(2048).optional().describe("Optional group hyperlink URL."), decoration: z.string().max(80).optional().describe("Optional defined decoration style.") }).strict(),
...
const motionCommon = {
  ...
  steps: z.number().int().min(2).max(32).describe("Number of independently renderable static states, including endpoints."),
  easing: easing.optional(),
  name: z.string().min(1).max(80).optional().describe("Base name for generated layers/views."),
};
export const viewBuildSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("reveal").describe("Progressive layer/object reveal."), pageId: motionCommon.pageId, groups: z.array(z.array(z.object({ kind: z.enum(["layer", "object"]).describe("Reveal target entity kind."), id: entityIdSchema.describe("Exact target layer or object ID.") }).strict()).min(1).describe("Targets introduced together in one reveal step.")).min(1).max(32).describe("Ordered reveal groups."), cumulative: z.boolean().optional().describe("When true, each generated view retains all prior groups."), name: motionCommon.name }).strict(),
  z.object({ kind: z.literal("motion").describe("Discrete duplicate-based object motion."), pageId: motionCommon.pageId, objectIds: z.array(entityIdSchema).min(1).max(32).describe("Distinct objects moved together."), from: point.describe("Initial translation pose."), to: point.describe("Final translation pose."), steps: motionCommon.steps, easing: motionCommon.easing, name: motionCommon.name }).strict(),
