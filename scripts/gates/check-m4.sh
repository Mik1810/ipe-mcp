...
[[ -x "$ROOT/scripts/gates/check-m3.sh" ]] || fail "check-m3.sh missing or not executable"
bash "$ROOT/scripts/gates/check-m3.sh" || fail "M0-M3 gates"
...
node "$ROOT/scripts/probes/probe-m4-roundtrip.mjs" "$NATIVE" || fail "native references or nested identities"
