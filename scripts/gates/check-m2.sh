...
[[ -x "$ROOT/scripts/gates/check-m1.sh" ]] || fail "check-m1.sh missing or not executable"
bash "$ROOT/scripts/gates/check-m1.sh"
...
for fixture in "${fixtures[@]}"; do
  ...
  python3 "$ROOT/scripts/probes/probe-m1.py" "$fixture" "$canonical" >"$TMP_ROOT/semantic-$safe_name.json" || fail "semantic probe $relative"
