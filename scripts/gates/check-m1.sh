...
[[ -x "$ROOT/scripts/gates/check-m0.sh" ]] || fail "check-m0.sh missing or not executable"
bash "$ROOT/scripts/gates/check-m0.sh"
...
python3 "$ROOT/scripts/probes/probe-m1-pdf.py" effects "$TMP_ROOT/effects.pdf" >/dev/null || fail "effects PDF matrix"
ipetoipe -pdf -nozip "$FIXTURES/bbox-viewbbox-group-transform.ipe" "$TMP_ROOT/bbox.pdf" >/dev/null 2>&1 || fail "bbox PDF export"
python3 "$ROOT/scripts/probes/probe-m1-pdf.py" bbox "$TMP_ROOT/bbox.pdf" >/dev/null || fail "bbox/link PDF structure"
...
PY
    python3 "$ROOT/scripts/probes/probe-m1.py" "$fixture" "$TMP_ROOT/rt-$base" --native-copy "$native_path" >"$TMP_ROOT/probe-$base.json" || fail "M1 probe: $base"
  else
    python3 "$ROOT/scripts/probes/probe-m1.py" "$fixture" "$TMP_ROOT/rt-$base" >"$TMP_ROOT/probe-$base.json" || fail "M1 probe: $base"
  fi
done
