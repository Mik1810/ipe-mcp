...
[[ -x "$ROOT/scripts/gates/check-m2.sh" ]] || fail "check-m2.sh missing or not executable"
bash "$ROOT/scripts/gates/check-m2.sh"
...
for token in \
  ...
  grep -Fq "$token" "$ROOT/docs/milestones/core-m3.md" || fail "M3 contract token missing: $token"
done
