...
bash "$ROOT/scripts/check-m6.sh" || fail "M6 gate"
...
grep -q '| IpePresenter 7.2.30 | verified | ignored |' "$ROOT/docs/viewer-effects-m7.md" || fail "IpePresenter matrix"
for viewer in 'Adobe Acrobat' 'Okular' 'Evince' 'pdfpc'; do grep -q "| $viewer | untested | untested |" "$ROOT/docs/viewer-effects-m7.md" || fail "$viewer matrix"; done
grep -q '| Browser PDF viewers | degraded | ignored |' "$ROOT/docs/viewer-effects-m7.md" || fail "browser matrix"
