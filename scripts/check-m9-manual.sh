...
# Every example in docs/m9-agent-manual.md is exercised against the candidate.
(cd "$ROOT" && node scripts/m9-agent-workflow.mjs "$M9_MANUAL_TMP/manual") > "$M9_MANUAL_TMP/evidence.json" || fail "agent workflow exercise"

python3 - "$M9_MANUAL_TMP/evidence.json" "$ROOT/docs/m9-agent-manual.md" <<'PY' || fail "manual evidence audit"
