...
(cd "$ROOT" && node scripts/sbom.mjs "$M9_SBOM_TMP/regenerated.json") || fail "sbom generation"
cmp "$M9_SBOM_TMP/regenerated.json" "$ROOT/docs/m9-sbom.json" || fail "SBOM is not byte-deterministic"
...
python3 - "$ROOT/docs/m9-sbom.json" "$ROOT/docs/core-m9-sbom.md" <<'PY' || fail "SBOM coverage audit"
