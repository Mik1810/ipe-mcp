...
bash "$ROOT/scripts/check-m7.sh" || fail "M7 gate"
(cd "$ROOT" && npm run build) || fail "build"
(cd "$ROOT" && npm test -- --run tests/mcp) || fail "MCP tests"
(cd "$ROOT" && node scripts/m8-sdk-host.mjs "$M8_TMP/scenario" > "$M8_TMP/sdk-host.json") || fail "independent real-stdio portable scenario"
...
audit=(root/'docs/m8-agentic-harness-audit.md').read_text(); assert audit.count('| PASS |')==24 and audit.count('| NOT APPLICABLE |')==7 and 'DEFERRED M9: 0' in audit
