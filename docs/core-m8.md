...
`tests/mcp` checks contracts, rollback/CAS, exact IDs, confirmations/evidence, persistent history, real SDK stdio, progress, native failure framing, resources, redaction, parity, and shutdown. `scripts/m8-sdk-host.mjs` runs the portable native scenario through an independent real MCP client and validates `.ipe`, PDF, PNG, rollback, undo/restore, and explicit resource reads. `scripts/check-m8.sh` recursively runs M7, M8 tests, the portable scenario, manifest/audit/config checks, and retained real-host evidence.
