...
The automated independent host is `scripts/host/m8-sdk-host.mjs`, built on `@modelcontextprotocol/client` 2.0.0 and a spawned stdio transport—not an in-process controller mock:

```bash
node scripts/host/m8-sdk-host.mjs /tmp/ipe-m8-sdk
```
