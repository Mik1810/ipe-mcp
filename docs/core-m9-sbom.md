...
`package-lock.json` and the installed `dpkg` metadata by `scripts/sbom.mjs`
and committed as [`m9-sbom.json`](./m9-sbom.json) (CycloneDX 1.5).
...
```bash
node scripts/sbom.mjs docs/m9-sbom.json      # regenerate (byte-deterministic)
bash scripts/check-m9-sbom.sh                # gate: determinism + coverage
```
...
temporary directory and asserts it matches `docs/m9-sbom.json` exactly.
