...
architecture-level contracts live in `docs/core-*.md`, and the field guide for
*authoring* an MCP server lives in `guide.md`.

Every example is exercised against the M9 candidate by
`node scripts/m9-agent-workflow.mjs` (see [Verification](#verification)).
...
- Three modes exist ([`compatibility-modes.md`](./compatibility-modes.md)):
...
- `transition` assigns a PDF transition effect to existing views; effects are
  best-effort per viewer ([`viewer-effects-m7.md`](./viewer-effects-m7.md)).
...
The enforced contract is [`core-m9-limits.md`](./core-m9-limits.md). Highlights
...
| `NATIVE_UNAVAILABLE` / `NATIVE_CRASH` | Ipe toolchain missing or broken | `ipe_get_capabilities`; verify `SETUP-WSL.md`; run `scripts/check-m9-setup.sh`. |
...
```bash
node scripts/m9-agent-workflow.mjs /tmp/ipe-m9-manual
```
...
```bash
bash scripts/check-m9-manual.sh
```
